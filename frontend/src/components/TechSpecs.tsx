    ...
    <section className="py-20 bg-gray-50" ref={ref}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="max-w-4xl mx-auto bg-gray-900 rounded-xl p-6 overflow-x-auto">
            <pre className="text-sm text-gray-300">
              <code>{`// Initialize Proof of Creation API
const poc = new ProofOfCreation({
  apiKey: 'your-api-key',
  network: 'mainnet'
});

// Create authenticity certificate
const certificate = await poc.create({
  file: imageFile,
  metadata: {
    creator: 'Artist Name',
    title: 'Artwork Title',
...
// Verify authenticity
const verification = await poc.verify(certificate.id);
console.log('Verification result:', verification.isValid);`}</code>
            </pre>
          </div>
        </motion.div>
